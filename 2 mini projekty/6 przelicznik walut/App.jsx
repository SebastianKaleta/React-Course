const Cash = props => {
  const value = ((props.cash / props.ratio) * props.prices).toFixed(2);
  return (
    <div>
      {props.title}
      {props.cash <= 0 ? "" : value}
...
class ExchangeCounter extends React.Component {
  ...
  static defaultProps = {
    ...
    prices: { electricity: 0.51, gas: 4.76, oranges: 3.79 }
  };
  handleChange = e => {
    this.setState({
  ...
  render() {
    const { amount, product } = this.state; //destrukturyzacja
    const price = this.selectPrices(product);
